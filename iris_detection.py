...
class iris_detection():
    ...
    def get_pupil(self):
        ...
        circle = cv.HoughCircles(self.gimg, cv.HOUGH_GRADIENT, 1, 300, param1=80, param2=0.8, minRadius=0, maxRadius=35)
    ...
    def get_iris(self):
        ...
        circle = cv.HoughCircles(self.gimg, cv.HOUGH_GRADIENT, 1, 300, param1=80, param2=0.85, minRadius=35, maxRadius=91)
        circle = np.uint16(np.around(circle))
        for i in circle[0,:]:
            if( abs(self.pupil[0] - int(i[0])) > 11 or abs(self.pupil[1] - int(i[1])) > 11): # sets iris size to 2*pupil if center is too far
                i[0] = self.pupil[0]
                i[1] = self.pupil[1]
                i[2] = 2.6*self.pupil[2]
                cv.circle(self.cimg, (i[0],i[1]), i[2], (0,255,0), 2)
                cv.circle(self.cimg, (i[0],i[1]), 2, (0,0,255), 2)
            else:
                if( int(i[2]) > 2.6*self.pupil[2] ): # sets iris size to 2.5*iris size if detected size is greater than 2.5*iris size
                    i[2] = 2.6*self.pupil[2]
                # draw the outer circle
                cv.circle(self.cimg, (i[0],i[1]), i[2], (0,255,0), 2)
                # draw the center of the circle
